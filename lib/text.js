...
function lengthLis(nOutGroups, maxSce) { 
  return [3, maxSce, 11 + nOutGroups.toString().length, 7];
}
function hdrLis(nOutGroups, fields) { 
  return [fields[0], fields[1], fields[2].replace('X', nOutGroups.toString()), fields[3]];
}
...
const self = module.exports = {
  ...
  prRepHdr: (text, root, ofile, fields, nGroups, maxSce) => {
    ...
    self.prColHdrs(hdrLis(nGroups, fields), lengthLis(nGroups, maxSce), ofile, 2)
  },
  prRepFtr: (text, ofile) => {
    prLine('', ofile);
    self.prHeading(text, ofile, 0);
  ...
  prRepLin: (sfile, sceLin, ofile, nGroups, maxSce) => {
    const fields = [sceLin.sceNum, sceLin.sce, sceLin.nFail, sceLin.status];
    self.prRec(fields, lengthLis(nGroups, maxSce), ofile, 2);
  },
  prSceHdr: (text, ofile) => {
    prLine('', ofile);
    self.prHeading(text + ' {', ofile, 0);
