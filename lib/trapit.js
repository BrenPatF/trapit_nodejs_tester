...
const ROOT_HDRS = ['#', 'Scenario', 'Fails', 'Groups', 'Status'];
...
function prUTResults(results, ext, p, rootFolder) {
  ...
  p.prRepHdr(results.repHdr, rootFolder + root, ifile, ROOT_HDRS, maxSce);
  for (const s in results.sceLis)  {
    ...
    p.prRepLin(sfile, sceObj.sceLin, ifile, maxSce);
