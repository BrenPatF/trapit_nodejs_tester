...
function expRepRegex(delimiter, exp, act) {
  const [expLis, actLis] = [csvToLis(delimiter, exp), csvToLis(delimiter, act)];
...
function mkInpObj(delimiter, inp, groups) {
  let inpGroups = {};
  let i = 0;
  let csvToLisD = x => csvToLis(delimiter, x);
  for (let g in groups)  {
...
function prUTResults(results, ext, p, rootFolder) {
  ...
  p.prRepHdr(results.repHdr, rootFolder + root, ifile, ROOT_HDRS, results.nGroups, maxSce);
  for (const s in results.sceLis)  {
    ...
    p.prRepLin(sfile, sceObj.sceLin, ifile, results.nGroups, maxSce);
  ...
  *************************************************************************************************/
  getUTResults: (meta, scenarios) => {
    ...
    return {
        title   : meta.title,
        sceLis  : sceLis,
        nGroups : Object.keys(meta.out).length,
        result  : result,
