...
function valWidths(time_width, time_dp, time_ratio_dp, calls_width = 5) {
  if (calls_width < 5) 
    ...
    throw new Error('Error, ' + 'time_width, time_dp, time_ratio_dp must be > 0, actual: ' + time_width + ', ' + time_dp + ', ' + time_ratio_dp);
  else if (+time_width + +time_dp < 7)
    throw new Error('Error, ' + 'time_width + time_dp must be > 6, actual: ' + time_width + ' + ' + time_dp);
  else if (+time_width + +time_ratio_dp < 8)
    throw new Error('Error, ' + 'time_width + time_ratio_dp must be > 7, actual: ' + time_width + ' + ' + time_ratio_dp);
}
/***************************************************************************************************
...
***************************************************************************************************/
function timerLine(timer, maxName, ela, usr, sys, calls, time_width, time_dp, time_ratio_dp, calls_width) {
  ...
  getTimers() {
    if (this.results.length === 0) {
      ...
      const sumTim = Array.from(this.timerHash.values()).reduce (function(t, s) {
        return {ela : t.ela + s.ela, usr : t.usr + s.usr, sys : t.sys + s.sys, calls : t.calls + s.calls}; });

      this.results = Array.from(this.timerHash.entries()).map(
        e => {return {timer: e[0], ela: e[1].ela, usr: e[1].usr, sys: e[1].sys, calls: e[1].calls};});
      this.results.push({timer: OTH_TIMER, ela: totTim.ela - sumTim.ela, usr: totTim.usr - sumTim.usr, sys: totTim.sys - sumTim.sys, calls: 1});
      this.results.push({timer: TOT_TIMER, ela: totTim.ela, usr: totTim.usr, sys: totTim.sys, calls: sumTim.calls + 1});
    }
    return this.results;
  }
  /***************************************************************************************************

  formatTimers: Writes the timers to an array of strings for the timer set

  ***************************************************************************************************/
  formatTimers(time_width = TIME_WIDTH, time_dp = TIME_DP, time_ratio_dp = TIME_RATIO_DP, calls_width = CALLS_WIDTH) {
    ...
    for (const i in timerArr) {
      const t = timerArr[i];
      fmtArr.push(timerLine(t.timer, maxName, t.ela, t.usr, t.sys, t.calls, +time_width, +time_dp, +time_ratio_dp, +calls_width));
  ...
  static getSelfTimer(self_range = SELF_RANGE) {
    ...
    return {ela: timerTimes.ela/self_range, usr: timerTimes.usr/self_range, sys: timerTimes.sys/self_range};
  }
  static formatSelfTimer(self_range = SELF_RANGE, time_width = TIME_WIDTH, time_dp = TIME_DP, time_ratio_dp = TIME_RATIO_DP) {
    valWidths(time_width, time_dp, time_ratio_dp);
    const t = this.getSelfTimer();
    return '[Timer timed (per call in ms): Elapsed: ' + formTimeTrim(TIME_FACTOR*t.ela, time_ratio_dp, time_width) + ', USR: ' + 
      formTimeTrim(TIME_FACTOR*t.usr, time_ratio_dp, time_width) + ', SYS: ' +  formTimeTrim(TIME_FACTOR*t.sys, time_ratio_dp, time_width) + ']';
  }
  formatResults(time_width = TIME_WIDTH, time_dp = TIME_DP, time_ratio_dp = TIME_RATIO_DP, calls_width = CALLS_WIDTH, self_range = SELF_RANGE) {
    valWidths(time_width, time_dp, time_ratio_dp, calls_width);
