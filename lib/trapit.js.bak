...
const ROOT_HDRS = ['#', 'Scenario', 'Fails (of X)', 'Status'];
...
***************************************************************************************************/
function _mkUTResultsFolder(results, ext, p, rootFolder) {
  const root = results.result.resFolder,
        ifile = root + ext;

  const maxSce = Math.max(ROOT_HDRS[1].length, Object.keys(results.sceLis).reduce( (a, b) => { return a.length > b.length ? a : b; }).length);
  p.prRepHdr(results.repHdr, rootFolder + root, ifile, ROOT_HDRS, results.nOutGroups, maxSce);
  for (const s in results.sceLis)  {
    ...
    p.prSceHdr(sceObj.sceHdr, sfile);
    ...
    p.prRepLin(sfile, sceObj.sceLin, ifile, results.nOutGroups, maxSce);
...
const self = module.exports = {
  ...
  *************************************************************************************************/
  mkUTResultsFolder: (meta, scenarios, formatType = 'B', rootFolder = './') => {
    const results = self.getUTResults(meta, scenarios);

    if (formatType == 'H' || formatType == 'B') {_mkUTResultsFolder(results, '.html', Html, rootFolder);}
